...
def query_rag(subject, question, top_k, category_filter, use_reranker):
    ...
    try:
        ...
        return (
            f"**Error:** {str(e)}\n\nCheck that ChromaDB data directory is accessible.",
            "",
            "",
            "",
        )
...
.tab-nav > button:hover { background: #d1d5db !important; }
...
.gr-button:hover { background: #d1d5db !important; }
.gr-button:active {
...
<div class="app-header">
    <h1>NoteBase</h1>
    <p>Organize, ingest, and query your knowledge base — each subject is isolated with its own ChromaDB collection.</p>
    <br>
...
with gr.Blocks(title="NoteBase") as demo:
    gr.HTML(HEADER_HTML)
                    ...
                    with gr.Tabs():
                        ...
                        with gr.Tab("Context"):
                            context_output = gr.Markdown(
                                value="Raw context chunks from ChromaDB will appear here."
                            )
...
if __name__ == "__main__":
    logger.info(f"Starting NoteBase on {APP_HOST}:{APP_PORT}")
