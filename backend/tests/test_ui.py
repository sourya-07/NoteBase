...
history_file = os.path.join(current_dir, "test_docs", "history.txt")
science_file = os.path.join(current_dir, "test_docs", "science.txt")

# Set the port dynamically from config/env or use 7860
port = os.getenv("APP_PORT", "7860")

print("Waiting for app to start...")
time.sleep(2)

print(f"Connecting to local Gradio client on port {port}...")
...
res3 = client.predict(
    subject="History",
    files=[handle_file(history_file)],
    links_text="",
    reset=True,
...
res4 = client.predict(
    subject="Science",
    files=[handle_file(science_file)],
    links_text="",
    reset=True,
    api_name="/ingest_subject_docs"
)
